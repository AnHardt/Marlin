...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_CONTAINS(0, STATUS_SCREENHEIGHT + 1)) {
...
*/
  ...
  void lcd_implementation_drawedit(const char* const pstr, const char* const value=NULL) {
    ...
    uint8_t baseline = segmentHeight + (DOG_CHAR_HEIGHT_EDIT) / 2;

    if (PAGE_CONTAINS(baseline + 1 - 8, baseline)) {
